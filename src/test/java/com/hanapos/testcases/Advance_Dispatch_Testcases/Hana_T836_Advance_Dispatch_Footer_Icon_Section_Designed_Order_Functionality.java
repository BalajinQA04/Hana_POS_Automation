...
public class Hana_T836_Advance_Dispatch_Footer_Icon_Section_Designed_Order_Functionality extends TestBaseClass {
    ...
    public void Validate_Hana_T836_Advance_Dispatch_Footer_Icon_Section_Designed_Order_Functionality(
            String username,
            String password,
            String shopname,
            String salesperson,
        ...
        try {
            ...
            phoneorder.SearchAndSelectReciAddress1("1160 W 5th St, Marysville, OH, USA");
            delayWithGivenTime(2000);
            ...
            softassert.assertEquals(phoneorder.getReciZipcode(), "43040", "Test Step - 4 - Recipient address 2 is not matched with customer address 2 on phone order page recipient section");
            softassert.assertEquals(phoneorder.getReciCity(), "Marysville", "Test Step - 4 - Recipient city is not matched with customer city on phone order page recipient section");
            softassert.assertEquals(phoneorder.getRecipientState(), "OH", "Test Step - 4 - Recipient phone number is not matched with customer phonenumber 1 field on phone order page recipient section");
            ...
            softassert.assertEquals(dashboardorder.validate_Designed_label_Status_On_AllOrdersPage(invoiceNumber), "Designed: " + getAtlanticTime_NumberDateFormat(), "Test Step - 10: Designed label is not displayed for designed order");

            //=========== Non - Designed Delivered Order ===========//
            delayWithGivenTime(2000);
            softassert.assertTrue(dashboard.VerifyOrderEntryOptionIsDisplayed(), "Test Step - 2: Order entry option is not displayed");
            softassert.assertTrue(dashboard.Verify_Cashandcarry_OptionIsDisplayed(), "Test Step - 2: Cash and carry option is not displayed");
            ...
            dashboardorder.EnterGlobalSearch(second_invoiceNumber);
            delayWithGivenTime(1000);
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(second_invoiceNumber), "Test Step - 10 - Respective Invoice number " + second_invoiceNumber + " is not displayed on all orders page");
            ...
            // Test Step - 14
            softassert.assertTrue(advanceDispatch.is_Invoice_Number_Displayed_In_Pending_Deliveries_Section(invoiceNumber), "Test Step - 14 - Invoice: " + invoiceNumber + " is not displaying in the pending Deliveries section");

            // Test Step - 15
            softassert.assertTrue(advanceDispatch.verify_I_Icon_For_Invoice_Is_Displayed(invoiceNumber), "Test Step - 15 - I icon is not displayed for invoice number: " + invoiceNumber);
            ...
            softassert.assertTrue(advanceDispatch.verify_Designed_Icon_IsDisplayed_On_Pending_Deliveries(invoiceNumber), "Test Step - 15 - Designed icon is not displayed for invoice number: " + invoiceNumber);

            // Test Step - 16
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatch.verify_I_Icon_For_Invoice_Is_Displayed(second_invoiceNumber), "Test Step - 16 - I icon is not displayed for invoice number: " + second_invoiceNumber);
            delayWithGivenTime(2000);
            advanceDispatch.click_I_Icon_For_Invoice_Is_Displayed(second_invoiceNumber);
            // softassert.assertTrue(advanceDispatch.verify_Highlighted_ToolTip_On_Map_View_IsDisplayed(), "Test Step - 16 - Highlighted tooltip is not displayed on map view");
            softassert.assertTrue(advanceDispatch.verify_Designed_Icon_Is_NotDisplayed_On_Pending_Deliveries(second_invoiceNumber), "Test Step - 16 - Non - Designed icon is displayed for invoice number: " + second_invoiceNumber + " which is not configured the timed delivery feature");
