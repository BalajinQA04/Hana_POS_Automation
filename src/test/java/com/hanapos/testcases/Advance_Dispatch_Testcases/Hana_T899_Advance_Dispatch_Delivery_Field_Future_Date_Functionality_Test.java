...
public class Hana_T899_Advance_Dispatch_Delivery_Field_Future_Date_Functionality_Test extends TestBaseClass {
    ...
    public void Validate_Hana_T899_Advance_Dispatch_Delivery_Field_Future_Date_Functionality_Test(
            String username,
            String password,
            String shopname,
            String salesperson,
        ...
        try {
            ...
            delayWithGivenTime(2000);
          //  softassert.assertEquals(advanceDispatchPage.get_Displayed_Delivery_Date_On_Advance_Dispatch(), automationshop_deliverydate, "Test Step - 7: By default delivery date is not displayed as current date " + CurrentDate() + " on advance dispatch page");
            ...
            delayWithGivenTime(3000);
            softassert.assertTrue(advanceDispatchPage.validate_the_invoice_is_displaying_in_the_trip_section(invoice_number), "Invoice Order in trip section");
            delayWithGivenTime(2000);

            advanceDispatchPage.Select_Driver(prop.getProperty("Assigned_DriverName"));
            softassert.assertEquals(advanceDispatchPage.get_selected_drivername(), prop.getProperty("Assigned_DriverName"), "Test Step - 9 - Selected Driver Is not displaying");
            delayWithGivenTime(2000);

            advanceDispatchPage.Click_DispatchSave_Button();
            phoneorder = new OrderEntry_Alais_PhoneOrderPage();
            delayWithGivenTime(1500);
            softassert.assertTrue(phoneorder.verifySuccessToastMessageAppears(), "Test Step - 10 - Success Toaster Message is not displaying");
            softassert.assertEquals(phoneorder.verifySuccessToastMessageText(), "Dispatch Saved Successfully", "Test Step - 10 - Delivery Date should be greater than today - Toaster Message is not displayed for future date invoice");
