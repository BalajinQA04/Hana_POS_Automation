...
public class Hana_T959_Advance_Dispatch_Trip_Section_New_Route_Functionality_Test extends TestBaseClass {
    ...
    String dispatchedTime_secondorder;
    String dispatchedTime;
    public static final String dataSheetName = "Hana_T944";
    ...
    public void Validate_Hana_T959_Advance_Dispatch_Trip_Section_New_Route_Functionality_Test(
            String username,
            String password,
            String shopname,
            String salesperson,
...
//======================================= Second Invoice Number =====================
            ...
            softassert.assertTrue(orderconfirmationpage.VerifyOrderConfirmationPage(), "Test Step - 5 - Order confirmation page is not displayed");
            delayWithGivenTime(500);
            second_invoice_number = orderconfirmationpage.get_invoiceNumber_on_OrderConfirmation_Page();
            ...
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(second_invoice_number), "Test Step - 6 - Respective Invoice number is not displayed on all orders page");
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(second_invoice_number), "New", "Test Step - 6 - Respective Invoice status is not displayed on all orders page");
            delayWithGivenTime(2000);
            ...
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.verify_pending_deliveries_invoice_isDisplayed(invoice_number), "Test Step - 14 - Invoice : " + invoice_number + " is not displaying in the pending Deliveries section");
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.verify_pending_deliveries_invoice_isDisplayed(second_invoice_number), "Test Step - 14 - Invoice : " + second_invoice_number + " is not displaying in the pending Deliveries section");
            ...
            dispatchedTime = Atlantic_TimeZone_In_MM_dd_yyyy_hhmm();
            phoneorder = new OrderEntry_Alais_PhoneOrderPage();
            ...
            softassert.assertEquals(advanceDispatchPage.get_Actual_TimeStamp_Displayed_On_Trip_Section(), dispatchedTime, "Test Step - 16: Time Stamp on trip section - Advance dispatch page");
            ...
            softassert.assertTrue(advanceDispatchPage.verify_pending_deliveries_invoice_isDisplayed(second_invoice_number), "Test Step - 18 - Invoice is not displaying in the pending Deliveries section");

            // Test Step - 19
            advanceDispatchPage.Click_pending_deliveries_invoice(second_invoice_number);
            delayWithGivenTime(2000);

            // skip the multiple order with same address pop-up
            if (advanceDispatchPage.multiple_order_with_same_address_popup_is_displayed()) {
                advanceDispatchPage.uncheck_the_ivoice_remaining_the_invoice_required_in_multiple_invoices_with_same_address(second_invoice_number);
                advanceDispatchPage.Click_confirm_button_in_multiple_order_with_same_address_popup();
            }

            delayWithGivenTime(3000);
            softassert.assertTrue(advanceDispatchPage.validate_the_invoice_is_displaying_in_the_trip_section(second_invoice_number), "Test Step - 19 - Invoice: " + second_invoice_number + " is not displaying in the Trip Section");

            // Test Step - 19
            advanceDispatchPage.Select_Driver(prop.getProperty("Assigned_DriverName"));
            softassert.assertEquals(advanceDispatchPage.get_selected_drivername(), prop.getProperty("Assigned_DriverName"), "Test Step - 19 - Selected Driver Is not displaying");

            //Test Step - 19
            advanceDispatchPage.Click_DispatchSave_Button();
            dispatchedTime_secondorder = Atlantic_TimeZone_In_MM_dd_yyyy_hhmm();
            phoneorder = new OrderEntry_Alais_PhoneOrderPage();
            delayWithGivenTime(1500);
            softassert.assertTrue(phoneorder.verifySuccessToastMessageAppears(), "Test Step - 19 - Success Toaster Message is not displaying");
            second_routenumber = advanceDispatchPage.get_Actual_Route_Number_Displayed_In_Trip_Section();
            delayWithGivenTime(2000);

            softassert.assertTrue(advanceDispatchPage.verify_Dispatch_Icon_On_Trip_Section(), "Test Step - 19: Dispatch yellow icon is not displayed on trip section");
            softassert.assertTrue(advanceDispatchPage.verify_Route_Number_IsDisplayed_In_Trip_Section(), "Test Step - 19: Route number along with driver name is not displayed on trip section");
            softassert.assertTrue(advanceDispatchPage.is_TimeStamp_Displayed_On_TripSection(), "Test Step - 19: Time Stamp on trip section - Advance dispatch page");
            softassert.assertEquals(advanceDispatchPage.get_Actual_TimeStamp_Displayed_On_Trip_Section(), dispatchedTime, "Test Step - 19: Time Stamp on trip section - Advance dispatch page");
            softassert.assertTrue(advanceDispatchPage.is_TripNumber_Displayed_On_Trip_Section(), "Test Step - 19: Trip number is not displayed on trip section");

            advanceDispatchPage.click_Route_Dropdown_In_Trip_Section();
            softassert.assertTrue(advanceDispatchPage.is_Existing_Trip_Displayed(routenumber), "Test Step - 19: Saved trip invoice number : " + invoice_number + " is not displayed in saved trip section");
            softassert.assertTrue(advanceDispatchPage.is_NewRoute_Option_Displayed_On_SavedTrip_Dropdown(), "Test Step - 19: New route option is not displayed on saved trip dropdown");
