...
public class Hana_T978_Advance_Dispatch_Trip_Section_Add_Payrate_Payable_Checked_Functionality_Test extends TestBaseClass {
    ...
    String second_invoice_number;
    String routenumber;
    String second_routenumber;
    public static final String dataSheetName = "Hana_T977";
    ...
    public void Validate_Hana_T978_Advance_Dispatch_Trip_Section_Add_Payrate_Payable_Checked_Functionality_Test(
            String username,
            String password,
            String shopname,
            String salesperson,
            ...
            String deliverytype,
            String mop,
            String nonpayable_reason,
            String driver_payrate,
            String deliverycode) {
        ...
        try {
            ...
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(invoice_number), "Test Step - 6 - Global search with Respective Invoice number: " + invoice_number + " is not displayed on all orders page");
            ...
            delayWithGivenTime(2000);
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.verify_pending_deliveries_invoice_isDisplayed(invoice_number), "Test Step - 14 - Invoice is not displaying in the pending Deliveries section");
            delayWithGivenTime(2000);
            ...
            softassert.assertTrue(advanceDispatchPage.is_New_Trip_Button_Displayed_Tripsection(), "Test Step - 16: New Trip button is not displayed on trip section");

            // Test Step - 19
            softassert.assertTrue(advanceDispatchPage.is_Add_Payrate_Button_Displayed(), "Test Step - 19: Remote Print button is not displayed on trip section");

            // Test Step - 20
            advanceDispatchPage.click_Add_Payrate_Button_Tripsection();
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.is_Add_Payrate_Popup_Displayed(), "Test Step - 20 - Add Payrate popup is not displayed");

            // Test Step - 21
            advanceDispatchPage.switchTo_Add_Payrate_TableGrid();
            delayWithGivenTime(2000);

            softassert.assertEquals(advanceDispatchPage.get_Displayed_InvoiceNumber_On_Add_Payrate_TableGrid(), invoice_number, "Test Step - 21 - Invoice number is not matched on add payrate table grid");

            // Test Step - 22
            advanceDispatchPage.enter_Driver_Payrate_On_Add_Payrate_Popup_TableGrid(driver_payrate);
            delayWithGivenTime(2000);
            softassert.assertEquals(advanceDispatchPage.get_Displayed_Driver_Payrate_On_Add_Payrate_TableGrid(), driver_payrate, "Test Step - 22 - Driver payrate is not matched on add payrate table grid");
            defaultContent();

            delayWithGivenTime(2000);
            advanceDispatchPage.click_Update_Button_On_Add_Payrate_Popup();
            delayWithGivenTime(2000);

            advanceDispatchPage.switchTo_Add_Payrate_TableGrid();
            delayWithGivenTime(2000);
            softassert.assertEquals(advanceDispatchPage.is_Payrate_Updated_Successfully_Label_Displayed(), "Pay Rate Updated Successfully", "Test Step - 20 - Add Payrate popup is not displayed");
            defaultContent();
            delayWithGivenTime(2000);

            // Test Step - 23
            advanceDispatchPage.click_Close_Icon_On_Add_Payrate_Popup();

            // Test Step - 24
            advanceDispatchPage.click_Add_Payrate_Button_Tripsection();
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.is_Add_Payrate_Popup_Displayed(), "Test Step - 23 - Add Payrate popup is not displayed");

            advanceDispatchPage.switchTo_Add_Payrate_TableGrid();
            delayWithGivenTime(2000);

            softassert.assertEquals(advanceDispatchPage.get_Displayed_InvoiceNumber_On_Add_Payrate_TableGrid(), invoice_number, "Test Step - 23 - Invoice number is not matched on add payrate table grid");

            advanceDispatchPage.enter_Non_Payable_Reason_On_Add_Payrate_Popup_TableGrid(nonpayable_reason);
            delayWithGivenTime(2000);
            softassert.assertEquals(advanceDispatchPage.get_Displayed_Driver_Payrate_On_Add_Payrate_TableGrid(), driver_payrate, "Test Step - 23 - Driver payrate is not matched on add payrate table grid");
            softassert.assertEquals(advanceDispatchPage.get_Displayed_Non_Payable_Reason_On_Add_Payrate_TableGrid(), nonpayable_reason, "Test Step - 23 - Non Payable Reason is not matched on add payrate table grid");
            defaultContent();
            ...
            advanceDispatchPage.switchTo_Add_Payrate_TableGrid();
            delayWithGivenTime(2000);
            softassert.assertEquals(advanceDispatchPage.is_Payrate_Updated_Successfully_Label_Displayed(), "Pay Rate Updated Successfully", "Test Step - 23 - Add Payrate popup is not displayed");
            defaultContent();
            delayWithGivenTime(2000);
            ...
            dashboard.ClickOrder();
            dashboardorder = new DashboardOrderPage();
            softassert.assertEquals(dashboardorder.validateDashboardOrderPage(), prop.getProperty("livedashboardorderURL"), "Test Step - 26 - All orders page is not displayed");

            // Test Step - 26
            delayWithGivenTime(2000);
            dashboardorder.EnterGlobalSearch(invoice_number);
            delayWithGivenTime(3000);
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(invoice_number), "Test Step - 26 - Respective Invoice number : " + invoice_number + " is not displayed on all orders page");
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(invoice_number), "Dispatched\nDriver: Liam Jack Benjamin", "Test Step - 26 - Order status is not displayed as delivered for cash and carry order");

            dashboard.MouseAndClick_Confirmation_sub_menu();
            confirmationPage = new ConfirmationPage();
            delayWithGivenTime(2000);
            softassert.assertTrue(confirmationPage.Verify_Delivery_Confirmation_Header(), "Test Step - 26 - Confirmation delivery date popup is not displayed");
            delayWithGivenTime(2000);
            ...
            confirmationPage.Enter_RecipientInvoice_ConfirmationPage(invoice_number);
            delayWithGivenTime(2000);
            softassert.assertEquals(confirmationPage.get_entered_RecipientInvoice_ConfirmationPage(), invoice_number, "Test Step - 26 - Recipient Invoice is not matched with entered invoice number on confirmation page");
            ...
            softassert.assertEquals(confirmationPage.get_selected_delivery_code_Confirmation_Page(), deliverycode, "Test Step - 26 - Selected delivery code is not displayed properly on confirmation page");

            confirmationPage.confirm_selected_deliveries_button_click_Confirmation_page();
            softassert.assertTrue(phoneorder.verifySuccessToastMessageAppears(), "Test Step - 26 - Delivery Confirmation Toaster message is not displayed");
            confirmationPage.confirm_delivery_page_close_icon_click();

            dashboard.ClickOrder();
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(invoice_number), "Test Step - 26 - Respective Invoice number is not displayed on all orders page");

            dashboardorder.EnterGlobalSearch(invoice_number);
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(invoice_number), "Test Step - 26 - Respective Invoice number is not displayed on all orders page");
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(invoice_number), "Delivered\nDriver: Liam Jack Benjamin", "Test Step - 26 - Order status is not displayed as delivered for cash and carry order");

            // Test Step - 27
            dashboard.MouseAndClick_Payroll_Reports_sub_menu();
            payrollreportpage = new Reports_PayrollReports_Page();
            softassert.assertTrue(payrollreportpage.verify_payroll_report_page_ISDisplayed(), "Test Step 28 - Payroll Report page is not displaying");

            //Test Step 28
            payrollreportpage.Enter_Delivery_From_Date(CurrentDate());
            payrollreportpage.Enter_Delivery_To_Date(automationshop_deliverydate);
            ...
            delayWithGivenTime(2000);
            softassert.assertTrue(payrollreportpage.is_InvoiceNumber_Displayed_On_Payroll_Report_Table_Grid(invoice_number), "Test Step 29 - Invoice: " + invoice_number + " is not displaying");

            //Test Step 30
            delayWithGivenTime(2000);
            softassert.assertTrue(payrollreportpage.is_Payable_Checkbox_Checked_On_Payroll_Report_Table_Grid(invoice_number), "Test Step 30 - Non Payable Reason checkbox is not checked as expected");
