...
public class Hana_T843_Advance_Dispatch_Select_Shops_Functionality_Test extends TestBaseClass {
    ...
    String secondshop_invoiceNumber;
    String automationshop_deliverydate;
    public static final String dataSheetName = "Hana_T843";
    ...
    public void Validate_Hana_T843_Advance_Dispatch_Select_Shops_Functionality_Test(
            String username,
            String password,
            String shopname,
            String canadasalesperson,
            String salesperson,
            String customername,
        ...
        try {
            ...
            // Test Step - 3
            phoneorder.Select_SalesPersonOn_PhoneOrderEntryPage(canadasalesperson);
...
//            phoneorder.Enter_ItemCodeRow1_Product(itemcode);
//            phoneorder.Enter_Product_ItemDescription_Row1(itemdescription);
//            phoneorder.Enter_Product_Qty_Row1(itemquantity);
//            phoneorder.Enter_Product_Price_Row1(itemprice);
//            delayWithGivenTime(2000);
...
//            softassert.assertEquals(phoneorder.getProdItemQty1OnPhoneOrderPage(), itemquantity, "Test Step - 3 - Item quantity is not displayed on phone order page product details section");
//            softassert.assertEquals(phoneorder.getUnitPriceOnProdDetails(), itemprice, "Test Step - 3 - Item price is not displayed on phone order page product details section");
//            delayWithGivenTime(2000);
...
//============================= Pre - requiste of Automation Shop order =========================================

            dashboard.ClickOnHomeIcon();
            dashboard.SelectShopNameDropDown(secondshop);
            softassert.assertEquals(dashboard.get_selected_shopname_from_hanadashboard(), secondshop, "Selected shop on dashboard page is not displayed");
            ...
            dashboard.ClickOrderEntry();
            phoneorder = new OrderEntry_Alais_PhoneOrderPage();
            phoneorder.Select_ShopName_On_PhoneOrder_Page(secondshop);
            delayWithGivenTime(2000);
            softassert.assertEquals(phoneorder.get_selected_shopname_on_phoneorder_page(), secondshop, "Test Step - 2 - Selected the shop name on phoneorder page is not displayed properly as expected");
            ...
            softassert.assertTrue(orderconfirmationpage.VerifyOrderConfirmationPage(), "Test Step - 5 - Order confirmation page is not displayed");
            delayWithGivenTime(500);
            secondshop_invoiceNumber = orderconfirmationpage.get_invoiceNumber_on_OrderConfirmation_Page();
            ...
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(secondshop_invoiceNumber), "Test Step - 6 - Respective Invoice number is not displayed on all orders page");
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(secondshop_invoiceNumber), "New", "Test Step - 6 - Respective Invoice status is not displayed on all orders page");
            ...
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.verify_pending_deliveries_invoice_isDisplayed(secondshop_invoiceNumber), "Test Step - 15 - Invoice is not displaying in the pending Deliveries section");

            advanceDispatchPage.Select_Shop_Dropdown_On_Advance_Dispatch_Page(shopname);
            delayWithGivenTime(3000);
            softassert.assertEquals(advanceDispatchPage.get_Selected_Shop_Dropdown_Value_Dispatch_Page(), shopname, "Test Step - 15: Selected shopname dropdown field value is not displayed");

            if (advanceDispatchPage.verifying_invalid_address_pop_up_is_displaying_or_not()) {
                PressEscapeKey();
            }
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.verify_pending_deliveries_invoice_isDisplayed(invoiceNumber), "Test Step - 15 - Invoice is not displaying in the pending Deliveries section");

            advanceDispatchPage.click_pending_deliveries_invoice_isDisplayed(invoiceNumber);
            ...
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(invoiceNumber), "New", "Test Step - 15 - Order status is not displayed as New");
