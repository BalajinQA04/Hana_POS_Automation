...
public class Hana_T867_Advance_Dispatch_Map_View_Only_CurrentTrip_wPending_Deliveries_Functionality_Test extends TestBaseClass {
    ...
    String pending_delivery_invoiceNumber;
    String current_trip_invoiceNumber;
    String automationshop_deliverydate;
    public static final String dataSheetName = "Hana_T867";
    ...
    public void Validate_Hana_T867_Advance_Dispatch_Map_View_Only_CurrentTrip_wPending_Deliveries_Functionality_Test(
            String username,
            String password,
            String shopname,
            String salesperson,
        ...
        try {
            ...
            phoneorder.SearchAndSelectReciAddress1("Rutland Drive, Carmichael");
            delayWithGivenTime(2000);
            ...
            softassert.assertEquals(phoneorder.getRecipientState(), "CA", "Test Step - 3 - Recipient phone number is not matched with customer phonenumber 1 field on phone order page recipient section");
            ...
            pending_delivery_invoiceNumber = orderconfirmationpage.get_invoiceNumber_on_OrderConfirmation_Page();
            ...
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(pending_delivery_invoiceNumber), "Test Step - 6 - Respective Invoice number is not displayed on all orders page");
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(pending_delivery_invoiceNumber), "New", "Test Step - 6 - Respective Invoice status is not displayed on all orders page");
...
//=================== Pre-requiste for Current trip ================================
            ...
            softassert.assertTrue(orderconfirmationpage.VerifyOrderConfirmationPage(), "Test Step - 5 - Order confirmation page is not displayed");
            delayWithGivenTime(500);
            current_trip_invoiceNumber = orderconfirmationpage.get_invoiceNumber_on_OrderConfirmation_Page();
            ...
            softassert.assertTrue(dashboardorder.validate_InvoiceNumber_on_AllOrdersPage(current_trip_invoiceNumber), "Test Step - 6 - Respective Invoice number is not displayed on all orders page");
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(current_trip_invoiceNumber), "New", "Test Step - 6 - Respective Invoice status is not displayed on all orders page");
            ...
            delayWithGivenTime(2000);
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.verify_pending_deliveries_invoice_isDisplayed(current_trip_invoiceNumber), "Test Step - 14 - Invoice is not displaying in the pending Deliveries section");

            // Test Step - 15
            advanceDispatchPage.Click_pending_deliveries_invoice(current_trip_invoiceNumber);
            delayWithGivenTime(2000);
            ...
            if (advanceDispatchPage.multiple_order_with_same_address_popup_is_displayed()) {
                advanceDispatchPage.uncheck_the_ivoice_remaining_the_invoice_required_in_multiple_invoices_with_same_address(current_trip_invoiceNumber);
                advanceDispatchPage.Click_confirm_button_in_multiple_order_with_same_address_popup();
            }
            delayWithGivenTime(3000);
            softassert.assertTrue(advanceDispatchPage.validate_the_invoice_is_displaying_in_the_trip_section(current_trip_invoiceNumber), "Invoice Order in trip section");
            ...
            delayWithGivenTime(2000);
            advanceDispatchPage.Select_the_Map_View_on_Advance_Dispatch_Page(mapview);
            delayWithGivenTime(2000);
            softassert.assertEquals(advanceDispatchPage.get_Selected_Map_View_on_Advance_Dispatch_Page(), "Current Trip w/ Pending Deliveries  ", "Test Step - 18: Selected map view is not displayed on advance dispatch page");

//            advanceDispatchPage.click_Left_Arrow_On_Pending_Deliveries_Section();
//            advanceDispatchPage.click_Right_Arrow_On_Trip_Section();
            advanceDispatchPage.zoom_out_google_map();
            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.verify_Dispatched_Icon_IsDisplayed_On_MapView(), "Test Step - 18: Dispatch icon of current trip is not displayed");

            delayWithGivenTime(2000);
            softassert.assertTrue(advanceDispatchPage.verify_I_Icon_For_Invoice_Is_Displayed(pending_delivery_invoiceNumber), "Test Step - 18 - I icon is not displayed for invoice number: " + pending_delivery_invoiceNumber);
            advanceDispatchPage.click_I_Icon_For_Invoice_Is_Displayed(pending_delivery_invoiceNumber);
            delayWithGivenTime(2000);
            //   softassert.assertTrue(advanceDispatchPage.verify_Highlighted_ToolTip_On_Map_View_IsDisplayed(), "Test Step - 18 - Highlighted tooltip is not displayed on map view");
