...
public class Hana_T131_Pickup_CustomerSection_Edit_Customer_CustomerDetails_OrderDetails_StatusSearchFilter_FT extends TestBaseClass {
    ...
    public void Validate_Hana_T131_Pickup_CustomerSection_Edit_Customer_CustomerDetails_OrderDetails_StatusSearchFilter_Functionality_Test() {
        ...
        try {
            ...
            softassert.assertEquals(phoneorder.get_Status_Orderdetailstab(), "New", "Test Step - 9 - Status search box entered data is not displayed in order details tab at customer details popup");
            ...
            softassert.assertEquals(phoneorder.get_Status_Orderdetailstab(), "Del", "Test Step - 11 - Status search box entered data is not displayed in order details tab at customer details popup");

            // Test Step - 12
            RobotPressEnter();
            ThreadWait(2000); // Bug
            softassert.assertTrue(phoneorder.Verify_Status_Orderdetailstab("Delivered"), " Known Issue - Test Step - 12 - Filtered status with 3 characters is not displayed on the order details tab at customer details popup");

            // Test Step - 13
            ThreadWait(1000);
            phoneorder.Enter_Status_OrderDetailsTab_CustomerDetailsPopup("False");
            ThreadWait(1000);
            softassert.assertEquals(phoneorder.get_Status_Orderdetailstab(), "False", "Test Step - 13 - Status search box entered data is not displayed in order details tab at customer details popup");

            RobotPressEnter();
            softassert.assertTrue(phoneorder.Verify_NoCustomerOrdersFoundMessage_Appears_OrderDetailsTab(), "Known Issue - Test Step - 13 - Filtered with invalid status is not displayed on the message like No Customer Order Found in order details tab at customer details popup");
