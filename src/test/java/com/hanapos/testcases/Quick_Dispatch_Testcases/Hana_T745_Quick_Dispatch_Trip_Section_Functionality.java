...
public class Hana_T745_Quick_Dispatch_Trip_Section_Functionality extends TestBaseClass {
    ...
    String invoice;
    public static final String dataSheetName = "Hana_T745";
    ...
    public void Validate_Hana_T745_Quick_Dispatch_Trip_Section_Functionality(
        ...
        try {
          ...
          //  dispatch.Select_Delivery_Date_on_Quick_Dispatch_Page(CurrentDate());
            ...
            dispatch.Click_The_Invoice_In_The_Pending_Deliveries_Section(invoice);
            softassert.assertEquals(dispatch.get_InvoiceNumber_On_TripSection(), invoice, "Test Step 4 - Invoice number is not displaying in the Trip section");

            // Test Step - 5
            softassert.assertTrue(dispatch.trip_section_save_button_is_displayed(), "Test Step 5 - Save button is not displaying in the Trip Section");

            //Test Step - 6
            softassert.assertEquals(dispatch.Get_total_deliveries_count_from_trip_section(), "1", "Test step 6 - Count is not increased by 1");

            // Test Step - 7
            dispatch.Select_Driver(drivername);
            delayWithGivenTime(2000);
            softassert.assertEquals(dispatch.get_Selected_Driver(), drivername, "Test Step - 7 - Selected Driver is not displayed on the dropdown");
            ...
            softassert.assertTrue(dispatch.verify_trip_no_is_generated_or_not(), "Test step 8 - Trip number is not generated");
            softassert.assertTrue(dispatch.verify_route_number_is_generated_or_not(), "Test step 8 - Route number is not displaying");
            softassert.assertTrue(dispatch.verify_trip_section_created_date_time_format_displaying_properly_or_not(), "Test Step 8 - Created Date Time format is not displaying properly");

            // Test Step 9
            softassert.assertTrue(dispatch.Verify_NewTripBtn_IsDisplayed(), "Test Step - 9 - New trip button is not displayed");
            softassert.assertTrue(dispatch.Verify_RoutePlannerBtn_IsDisplayed(), "Test Step - 9 - Route planner button is not displayed");
            softassert.assertTrue(dispatch.Verify_RemotePrintBtn_IsDisplayed(), "Test Step - 9 - Remote print button is not displayed");
            softassert.assertTrue(dispatch.Verify_ManualPrintBtn_IsDisplayed(), "Test Step - 9 - Manual Print button is not displayed");
            softassert.assertTrue(dispatch.Verify_AddPayrateBtn_IsDisplayed(), "Test Step - 9 - Add Payrate button is not displayed");

            delayWithGivenTime(3000);

            // Test Step - 10
            softassert.assertTrue(dispatch.verify_whether_the_route_no_is_displaying_in_saved_trip_section(), "Route number is not displaying in the Saved Trip section");
