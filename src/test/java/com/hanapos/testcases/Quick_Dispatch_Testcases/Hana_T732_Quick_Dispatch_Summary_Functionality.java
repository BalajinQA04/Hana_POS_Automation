...
public class Hana_T732_Quick_Dispatch_Summary_Functionality extends TestBaseClass {
    ...
    public void Validate_Hana_T732_Quick_Dispatch_Summary_Functionality_Test(
        ...
        try {
            ...
            String sandiego_order_count = dispatch.Get_count_from_summarized_order_for_city_sandiego();

            dispatch.Click_San_diego_record_in_summary_table();
            delayWithGivenTime(1500);
            softassert.assertTrue(dispatch.validate_invoice_list_is_displaying_or_not_in_Trip_section(), "Test Step 4 - Summarized orders are not displaying in the trip section");
            String total_deliveries_count = dispatch.Get_total_deliveries_count_from_trip_section();
            delayWithGivenTime(1500);
            softassert.assertEquals(sandiego_order_count, total_deliveries_count, "Sandiego order count and the Total deliveries count is not equal");
        } catch (Exception e) {
            logger_Util = new LoggerUtil();
