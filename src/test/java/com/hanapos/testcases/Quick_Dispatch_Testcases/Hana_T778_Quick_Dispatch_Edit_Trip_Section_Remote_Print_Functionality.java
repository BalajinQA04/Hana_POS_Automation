...
public class Hana_T778_Quick_Dispatch_Edit_Trip_Section_Remote_Print_Functionality extends TestBaseClass {
    ...
    public void Validate_Hana_T778_Quick_Dispatch_Edit_Trip_Section_Remote_Print_Functionality(
        ...
        logger.info("**** Starting  Hana-Validate_Hana_T778_Quick_Dispatch_Edit_Trip_Section_Remote_Print_Functionality  ****");
        ...
        try {
            ...
            delayWithGivenTime(3000);
            softassert.assertEquals(dispatch.get_InvoiceNumber_On_TripSection(), invoice1, "Prerequiste steps - 3, Step 4 - Invoice number is not displaying in the Trip section");
            softassert.assertEquals(Integer.parseInt(dispatch.Get_total_deliveries_count_from_trip_section()), inital_delivery_count + 1, "Prerequiste steps - 3, Step 4 - Save button is not displaying in the Trip Section");

            dispatch.Select_Driver("LJB Liam Jack Benjamin");
            softassert.assertEquals(dispatch.get_Selected_Driver(), "LJB Liam Jack Benjamin", "Prerequiste steps - 3, Step - 4 - Selected Driver is not displayed on the dropdown");
            phoneorder = new OrderEntry_Alais_PhoneOrderPage();
            dispatch.Click_DispatchSave_Button();
            delayWithGivenTime(3000);
            softassert.assertEquals(phoneorder.verifySuccessToastMessageText(), "Dispatch Saved Successfully", "Prerequiste steps - 3, Step - 4 - Dispatch Saved Successfully toast success text message is not displayed");

            softassert.assertTrue(dispatch.verify_trip_no_is_generated_or_not(), "Prerequiste steps - 3, step 4 - Trip number is not generated");
            softassert.assertTrue(dispatch.verify_route_number_is_generated_or_not(), "Prerequiste steps - 3, step 4 - Route number is not displaying");
            softassert.assertTrue(dispatch.verify_trip_section_created_date_time_format_displaying_properly_or_not(), "Prerequiste steps - 3, Step 4 - Created Date Time format is not displaying properly");

            //Test Step 5
            softassert.assertTrue(dispatch.Verify_Trip_Section_IsDisplayed(), "Prerequiste steps - 3, Step 5 - Trip section is not displayed");
            ...
            softassert.assertEquals(dispatch.Get_Route_number(), Route_Number, "Prerequiste steps - 3, Step 5 - Trip section is displayed properly with respective route");
//   End of Prerequiste steps - 3 //

            //Test Step 4
            softassert.assertTrue(dispatch.Verify_RemotePrintBtn_IsDisplayed(), "Test Step 4 - Remote Print button is not displaying");

            //Test Step 5
            dispatch.Click_remote_print_button();

            softassert.assertTrue(phoneorder.verifySuccessToastMessageAppears(), "Test Step 5 - Toaster Message is not displaying");
            softassert.assertEquals(phoneorder.verifySuccessToastMessageText(), "Remote Print Completed Successfully");
