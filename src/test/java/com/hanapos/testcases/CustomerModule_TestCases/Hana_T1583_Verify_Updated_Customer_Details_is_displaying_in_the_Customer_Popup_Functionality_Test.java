...
public class Hana_T1583_Verify_Updated_Customer_Details_is_displaying_in_the_Customer_Popup_Functionality_Test extends TestBaseClass {
    ...
    @Owner("Balaji N")
    @Description("Hana_T :- Update Customer Details - Verify Updated Customer Details is displaying in the Customer Popup Functionality Test")
    @Epic("Customers Module")
    @Test(groups = {"Regression", "Sanity", "Smoke"})
    public void Validate_Hana_T1583_Verify_Updated_Customer_Details_is_displaying_in_the_Customer_Popup_Functionality_Test() {
        ...
        try {
            ...
            String updatecustFirstName = usFaker.name().firstName();
            String updatecustLastName = usFaker.name().lastName();
            String updatecustomerfullname = updatecustFirstName + " " + updatecustLastName;

            String updatecompanyname = Generate_CompanyName();
            String updatecustomerPhoneNumber = GenerateUsPhoneNumber();
            String updatealtPhoneNumber = GenerateCAPhoneNumber();
            String updatecustomerEmail = Generate_Random_EmailId();
            String updateusStreetAddress = usFaker.address().streetAddress();
            String updateusCity = usFaker.address().city();
            String updateusState = usFaker.address().state();
            String updateusZipCode = get_Random_US_ZipCode();
            String updateusFullAddress = usStreetAddress + ", " + usCity + ", " + usState + " " + usZipCode;
            String updatecanada_address = canadaFaker.address().fullAddress();
            String updatecity_state_zipcode = usCity + " " + usState + " " + usZipCode;

            customerpage.enter_CompanyName_On_Update_Customer_Details_Popup(updatecompanyname);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsCompanyNameTextBox(), updatecompanyname, "Test Step - 8 - Entered updated Company Name on customer details popup is not matched as expected");

            customerpage.enter_FirstName_On_Update_Customer_Details_Popup(updatecustFirstName);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsFirstNameTextBox(), updatecustFirstName, "Test Step - 8 - Entered updated first name on customer details popup is not matched as expected");

            customerpage.enter_LastName_On_Update_Customer_Details_Popup(updatecustLastName);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsLastNameTextBox(), updatecustLastName, "Test Step - 8 - Entered updated last name on customer details popup is not matched as expected");

            customerpage.enter_Address1_On_Update_Customer_Details_Popup(updatecanada_address);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsAddress1TextBox(), updatecanada_address, "Test Step - 8 - Entered updated address 1 on customer details popup is not matched as expected");

            customerpage.enter_Address2_On_Update_Customer_Details_Popup(usStreetAddress);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsAddress2TextBox(), usStreetAddress, "Test Step - 8 - Entered updated address 2 on customer details popup is not matched as expected");

            customerpage.enter_ZipCode_On_Update_Customer_Details_Popup(updateusZipCode);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsZipCodeTextBox(), updateusZipCode, "Test Step - 8 - Entered updated zipcode on customer details popup is not matched as expected");

            customerpage.enter_PhoneNumber_On_Update_Customer_Details_Popup(updatecustomerPhoneNumber);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsPhoneNumberTextbox(), updatecustomerPhoneNumber, "Test Step - 8 - Entered updated phone number on customer details popup is not matched as expected");

            customerpage.enter_AltPhoneNumber_On_Update_Customer_Details_Popup(updatealtPhoneNumber);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsAltPhoneNumberTextBox(), updatealtPhoneNumber, "Test Step - 8 - Entered updated alt phone number on customer details popup is not matched as expected");

            customerpage.enter_Email_On_Update_Customer_Details_Popup(updatecustomerEmail);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.getCustDetailsEmailTextBox(), updatecustomerEmail, "Test Step - 8 - Entered updated email id on customer details popup is not matched as expected");

            customerpage.enter_StoreCredit_On_Update_Customer_Details_Popup("100");
            delayWithGivenTime(1000);


            if (customerpage.is_CustomerDetails_Confirmation_Popup_Displayed()) {
                customerpage.Enter_ConfirmationCode_On_CustomerDetails_Confirmation_Popup(customerpage.get_Confirmation_Code_On_CustomerDetails_Confirmation_Popup());
                delayWithGivenTime(1000);
                customerpage.click_OK_Button_On_CustomerDetails_Confirmation_Popup();
            }

            softassert.assertEquals(customerpage.getCustDetailsStoreCreditTextBox(), "100", "Test Step - 8 - Entered updated credit card on customer details popup is not matched as expected");

            customerpage.select_Customer_Type_Dropdown_On_Update_Customer_Details_Popup("Florist");
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.get_Customer_Type_Dropdown_On_Update_Customer_Details_Popup(), "Florist", "Test Step - 8 - Selected updated customer type on customer details popup is not matched as expected");

            customerpage.select_Source_Code_Dropdown_On_Update_Customer_Details_Popup("Social Media");
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.get_Source_Code_Dropdown_On_Update_Customer_Details_Popup(), "Social Media", "Test Step - 8 - Source code dropdown is not displayed on customer details popup");

            customerpage.Enter_Customer_Notes_On_Update_Customer_Details_Popup("Automation Customer Notes");
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.get_Customer_Notes_On_Update_Customer_Details_Popup(), "Automation Customer Notes", "Test Step - 8 - Customer notes is not displayed on customer details popup");

            delayWithGivenTime(1000);
            customerpage.click_Update_Details_Button_On_Update_Customer_Details_Popup();
            delayWithGivenTime(1000);

            softassert.assertEquals(customerpage.verify_Toaster_Message_Text(), "Customer details updated successfully", "Test Step - 5: Customer details updated successfully - Success toaster message is not displayed");
            customerpage.click_Close_Icon_On_Success_Toaster_Message();
            delayWithGivenTime(3000);

            String customer_id = customerpage.get_Customer_ID_On_Add_New_Customer_Details_Popup();
            softassert.assertEquals(customerpage.get_Displayed_Customer_Name_On_Customer_Details_Popup_Header(), updatecustomerfullname, "Test Step - 8 - Customer Full Name on customer details popup header is not displayed properly");
            softassert.assertEquals(customerpage.get_Displayed_Customer_ID_On_Customer_Details_Popup_Header(), customer_id, "Test Step - 8 - Customer ID on customer details popup header is not displayed properly");
            softassert.assertEquals(customerpage.get_Displayed_Customer_Phonenumber_On_Customer_Details_Popup_Header(), updatecustomerPhoneNumber, "Test Step - 8 - Customer phone number on customer details popup header is not displayed properly");
            softassert.assertEquals(customerpage.get_Displayed_Customer_CompanyName_On_Customer_Details_Popup_Header(), updatecompanyname, "Test Step - 8 - Customer Company Name on customer details popup header is not displayed properly");
            delayWithGivenTime(2000);
            ...
            // Test Step - 11
            cashandcarry.EnterCustomerName(updatecustFirstName, updatecustomerfullname);
            softassert.assertEquals(cashandcarry.getDisplayedCustomerName(), updatecustomerfullname, "Test Step - 9 - Customer name is not matched with entered customer name");
            ...
            phoneorder.SearchAndSelectCustomerOnCust_Section(updatecustomerfullname);
            delayWithGivenTime(2000);
            softassert.assertEquals(phoneorder.getFirstnameOnPhoneOrderPage(), updatecustFirstName, "Test Step - 6 - First name is not displayed on phone order page");
            softassert.assertEquals(phoneorder.getLastnameOnPhoneOrderPage(), updatecustLastName, "Test Step - 6 - Last name is not displayed on phone order page");
            softassert.assertEquals(phoneorder.getCompanyNameOnPhoneOrderPage().equalsIgnoreCase(updatecompanyname), true, "Test Step - 6 - Company name is not displayed on phone order page");
            softassert.assertEquals(phoneorder.getEmailIdOnPhoneOrderPage(), updatecustomerEmail, "Test Step - 6 - email id is not displayed on phone order page");
            softassert.assertEquals(phoneorder.getAddress1OnPhoneOrderPage(), updatecanada_address, "Test Step - 6 - address 1 is not displayed on phone order page");
            softassert.assertEquals(phoneorder.getAddress2OnPhoneOrderPage(), usStreetAddress, "Test Step - 6 - Address 2 is not displayed on phone order page");
            softassert.assertEquals(phoneorder.getZipCodeOnPhoneOrderPage(), updateusZipCode, "Test Step - 6 - Zipcode is not displayed on phone order page");
            softassert.assertEquals(phoneorder.getPhoneNumberOnPhoneOrderPage(), updatecustomerPhoneNumber, "Test Step - 6 - phone number 1 is not displayed on phone order page");
            softassert.assertEquals(phoneorder.getAltPhoneNumberOnPhoneOrderPage(), updatealtPhoneNumber, "Test Step - 6 - Alt phone number is not displayed on phone order page");
