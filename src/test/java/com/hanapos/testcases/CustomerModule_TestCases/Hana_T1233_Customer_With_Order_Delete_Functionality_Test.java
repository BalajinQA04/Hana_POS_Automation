...
public class Hana_T1233_Customer_With_Order_Delete_Functionality_Test extends TestBaseClass {
    ...
    @Owner("Balaji N")
    @Description("Hana_T1233 :- Customer With Order Delete Functionality Test")
    @Epic("Customers Module")
    @Test(groups = {"Regression", "Sanity", "Smoke"}, dataProvider = "fetch_Excel_Data")
    public void Validate_Hana_T1233_Customer_With_Order_Delete_Functionality_Test(
            String username,
            String password,
            String shopname
        ...
        try {
            ...
            dashboard.click_Customer_Menu();
            customerpage = new CustomerPage();
            softassert.assertTrue(customerpage.VerifyCustomerMenuPage(), "Test Step - 20 - customer menu page is not displayed");

            customerpage.Select_ShopName_On_AddNewCustomer_Popup(shopname);
            softassert.assertEquals(customerpage.get_Selected_ShopName_On_Customer_Page(), shopname, "Test Step - 20 : Shop name is not matching with expected shop name as " + shopname);

            customerpage.search_Customer_On_Customer_Search_TextBox(custFirstName + " " + custLastName);
            delayWithGivenTime(1000);
            softassert.assertEquals(customerpage.VerifyCompanyNameOnCustTable(), companyname, "Test Step - 20 - Company Name on customer table is not matched");
            softassert.assertEquals(customerpage.VerifyPhoneNumberOnCustTable(), customerPhoneNumber, "Test Step - 20 - Phone number on customer table is not matched");
            softassert.assertEquals(customerpage.VerifyAddressOnCustTable(), usFullAddress, "Test Step - 20 - Address on customer table is not matched");
            softassert.assertEquals(customerpage.VerifyCityStateZipCodeOnCustTable(), city_state_zipcode, "Test Step - 20 - City, State & Zipcode on customer table is not matched");
            delayWithGivenTime(1000);
            ...
            softassert.assertEquals(customerpage.verify_Toaster_Message_Text(), "You can't delete this customer. Customer have order(s)", "Test Step - 20: Customer deleted successfully - Success toaster message is not displayed");
            customerpage.click_Close_Icon_On_Success_Toaster_Message();
