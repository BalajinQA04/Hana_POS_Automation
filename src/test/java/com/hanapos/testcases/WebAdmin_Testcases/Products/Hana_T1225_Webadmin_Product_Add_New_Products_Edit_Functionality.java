...
public class Hana_T1225_Webadmin_Product_Add_New_Products_Edit_Functionality extends TestBaseClass {
    ...
    public void Validate_Hana_T1225_Webadmin_Product_Add_New_Products_Edit_Functionality_Test() {
        ...
        try {
            ...
            String item_code = getRandomFlowerCode();
            ...
            delayWithGivenTime(1000);
            webadmin_productspage.click_Edit_Icon_On_Product_Details_TableGrid(item_name);
            delayWithGivenTime(2000);

            String edit_item_name = getRandomFlowerName();
            String edit_item_code = getRandomFlowerCode();
            String edit_product_variant = generate_Product_Variant();
            String edit_sku = generate_SKU_Code();
            String edit_price = generate_Price();

            webadmin_productspage.enter_Itemcode_On_BasicDetails_Tab(edit_item_code);
            webadmin_productspage.enter_Itemname_On_BasicDetails_Tab(edit_item_name);
            delayWithGivenTime(2000);
            webadmin_productspage.click_Pricing_Tab();
            delayWithGivenTime(2000);
            webadmin_productspage.click_Edit_Icon_Product_Variant1_On_Pricing_Tab();
            delayWithGivenTime(1000);
            webadmin_productspage.edit_Product_Variant1_On_Pricing_Tab(edit_product_variant);
            delayWithGivenTime(1000);
            webadmin_productspage.edit_SKU_On_Product_Variant1_On_Pricing_Tab(edit_sku);
            delayWithGivenTime(1000);
            webadmin_productspage.edit_Price_On_Product_Variant1_On_Pricing_Tab(edit_price);
            delayWithGivenTime(1000);
            webadmin_productspage.click_Save_Button_Product_Variant1_On_Pricing_Tab();
            delayWithGivenTime(3000);
            softassert.assertEquals(webadmin_productspage.validate_toaster_message_Text(), "Product Variant Updated.", "Test Step - 7: Product Variant Updated toaster success message is not displayed");

            delayWithGivenTime(2000);
            webadmin_productspage.click_Back_Button_On_Top();

            delayWithGivenTime(2000);
            webadmin_productspage.search_And_Select_Product(edit_item_name);
            delayWithGivenTime(1000);
            softassert.assertEquals(webadmin_productspage.get_Displayed_Itemcode_On_Product_Details_TableGrid(), edit_item_code, "Test Step - 9: Product item code is not displayed or not matched on product details table grid");
            softassert.assertEquals(webadmin_productspage.get_Displayed_Product_Name_On_Product_Details_TableGrid(), edit_item_name, "Test Step - 9: Product name is not displayed or not matched on product details table grid");
            softassert.assertEquals(webadmin_productspage.get_Displayed_Product_Price_On_Product_Details_TableGrid(), "$" + edit_price, "Test Step - 9: Product price is not displayed or not matched on product details table grid");
            ...
            // Test Step - 8
            cashandcarry.SearchAndSelectTheItemCode(edit_item_code, edit_item_name);
            softassert.assertEquals(cashandcarry.ItemCodeValueIsExists(), edit_item_code, "Test Step - 8 : Item code is not matched with search and selected item code");
            softassert.assertEquals(cashandcarry.ItemDescriptionValueIsExist(), edit_item_name + " " + edit_product_variant, "Test Step - 8 : Item description is not matched with search and selected item code");
            ...
            softassert.assertEquals(cashandcarry.ItemPriceValueIsExist(), edit_price, "Test Step - 8 : Item price is not matched with search and selected item code");
            ...
            softassert.assertEquals(cashandcarry.getAddedItemCode(), edit_item_code, "Test Step - 9 : Added item code is not matched with search and selected item code");
            softassert.assertEquals(cashandcarry.GetAddedItemDescription(), edit_item_name + " " + edit_product_variant, "Test Step - 9 : Added item description is not matched with search and selected item code");
            ...
            softassert.assertEquals(cashandcarry.GetAddedItemExtPrice(), "$" + edit_price, "Test Step - 9 : Added item extended price is not matched with search and selected item code");
            softassert.assertEquals(cashandcarry.GetAddedItemPrice(), "$" + edit_price, "Test Step - 9 : Added item price is not matched with search and selected item code");
