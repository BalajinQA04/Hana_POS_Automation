...
public class Hana_Orders_10_HanaPOS_Orders_CancelOrder_Credit_For_Wrong_Payment_Option_Functionality extends TestBaseClass {
    ...
    public void Validate_Hana_Orders_10_HanaPOS_Orders_CancelOrder_Credit_For_Wrong_Payment_Option_Functionality_Test(
        ...
        try {
            ...
            phoneorder.ClickPickupTypeOnPhoneOrderPage();
            delayWithGivenTime(2000);
            softassert.assertEquals(phoneorder.getReciFirstName(), prop.getProperty("cust_firstName"), "Test Step - 4 - Displayed first name is not matched with customer firstname on phone order page recipient section");
            softassert.assertEquals(phoneorder.getReciLastName(), prop.getProperty("cust_lastName"), "Test Step - 4 - Displayed last name is not matched with customer lastname on phone order page recipient section");
            softassert.assertEquals(phoneorder.getReciAddress1(), "114 N CHURCH ST", "Test Step - 4 - Recipient address 1 is not matched with customer address 1 field on phone order page recipient section");
            softassert.assertEquals(phoneorder.getReciZipcode(), "63084", "Test Step - 4 - Recipient address 2 is not matched with customer address 2 on phone order page recipient section");
            softassert.assertEquals(phoneorder.getReciCity().equalsIgnoreCase("UNION"), true, "Test Step - 4 - Recipient city is not matched with customer city on phone order page recipient section");
            ...
            phoneorder.EnterDeliveryDateOnReciSection(CurrentDate());
            phoneorder.Enter_DeliveryTime_OnRecipientSection(15, 25);
            phoneorder.Select_DeliveryOnTime_Dropdown("Starts At");
            ...
            phoneorder.Select_ProdSourceCode("Social Media");
            delayWithGivenTime(1000);
            softassert.assertEquals(phoneorder.get_selected_ProdSourceCode(), "Social Media", "Test Step - 9 - Selected product source code is not displayed on phone order page product details section");
...
//========================= Above are pre - requisite steps for this test case =============================================
            ...
            delayWithGivenTime(1000);
            dashboardorder.Select_CancelOrder_Reason("Credit: Wrong Payment Option");
            delayWithGivenTime(2000);
            softassert.assertEquals(dashboardorder.get_cancelOrder_Reason(), "Credit: Wrong Payment Option", "Test Step - 5 - Cancel order reason is not displayed on cancel order popup");
            ...
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(invoiceNumber), "Confirm Pickup\n" +
                    "\n" +
                    "Pending", "Test Step - 16 - Order status as pending is not displayed as pickup type order");
            ...
            // Test Step - 22
            String expectedDateTime = get_Current_AST_Time();
            String expectedTime = get_Current_AST_Time();
            ...
            softassert.assertTrue(
                    actualRefundMessage.contains("Refund Processed\nRefund Amount: $-" + total_amount + "\nRefund Method: Cash\nReason: Wrong Payment Option"),
                    "Refund message content mismatch for the Reason: Wrong Payment Option"
            );
