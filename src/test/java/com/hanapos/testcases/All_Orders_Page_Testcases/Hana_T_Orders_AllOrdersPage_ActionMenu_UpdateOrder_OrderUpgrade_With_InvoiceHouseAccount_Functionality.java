...
public class Hana_T_Orders_AllOrdersPage_ActionMenu_UpdateOrder_OrderUpgrade_With_InvoiceHouseAccount_Functionality extends TestBaseClass {
    ...
    String order_updated_date_time;
    ...
    public void Validate_Hana_T_Orders_AllOrdersPage_ActionMenu_UpdateOrder_OrderUpgrade_With_InvoiceHouseAccount_Functionality(
        ...
        try {
            ...
            delayWithGivenTime(1000);
            softassert.assertEquals(phoneorder.getDisplayedPaymentTypeSelectedOption(), prop.getProperty("payment_type_invoice"), "Test Step - 10 - Selected Payment type is not displayed on phone order page payment section");
            delayWithGivenTime(1000);
...
//========================= Above are pre - requisite steps for this test case =============================================
            ...
            // Test Step - 6
            // for payment reason dropdown field only available for cash payment types
...
//            softassert.assertEquals(dashboardorder.get_selected_paymentreason_updateorder_popup(), payment_reason, "Test Step - 6 - Update order reason is not displayed on update order popup");
            ...
            softassert.assertEquals(dashboardorder.get_MOP_On_Update_Order_Popup(), prop.getProperty("payment_type_invoice"), "Test Step - 7 - Selected Payment type is not displayed on update order popup");
            softassert.assertEquals(dashboardorder.get_Discount_On_Update_Order_Popup(), "$" + dashboardorder.expected_Discount_On_Update_Order_Popup(), "Test Step - 7 - Discount amount is not displayed on update order popup");
            softassert.assertEquals(dashboardorder.get_GrandTotal_On_Update_Order_Popup(), "$" + dashboardorder.get_Expected_GrandTotal_On_Update_Order_Popup(), "Test Step - 7 - Grand total amount is not displayed properly on update order popup");
            ...
            order_updated_date_time = get_Current_AST_Time();
            ...
            dashboardorder.EnterGlobalSearch(invoiceNumber);
            delayWithGivenTime(1000);
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(invoiceNumber), "New", "Test Step - 12 - Order status is not displayed as New on order page after clicking the yes button on update order popup");

            // Test Step - 12
            dashboardorder.clickInvoiceNumber_On_TableGrid_AllOrdersPage(invoiceNumber);
            delayWithGivenTime(2000);
            softassert.assertEquals(dashboardorder.get_OrderUpdated_ActiviesTab(), "ORDER UPDATE\n" +
                    order_updated_date_time + "\n" +
                    "Order Updated " + order_updated_date_time, "Test Step - 12 - Payment Processed label message is not displayed on activities tab order page");
