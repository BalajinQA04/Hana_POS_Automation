...
public class Hana_T1432_OrderPage_AllOrders_Verify_Status_As_New_For_Phone_Orders_Functionality_Test extends TestBaseClass {
    ...
    @Description("Hana_T1432 :- Verify Phone Order Status with existing customer functionality")
    @Test(enabled = true, groups = {"Regression", "Smoke", "Sanity"}, dataProvider = "fetch_Excel_Data")
    public void Validate_Hana_T1432_OrderPage_AllOrders_Verify_Status_As_New_For_Phone_Orders_Functionality_Test(
            String username,
            String password,
            String shopname,
            String salesperson,
        ...
        try {
            ...
            lp.ClickLoginButton();
            //   logPageLoad("Dashboard Page", () -> lp.ClickLoginButton());
            ...
            deliveryfee = phoneorder.getDeliveryFees_PaymentSection_OnPhoneOrder();
            relayfee = phoneorder.get_RelayFees_PaymentSection_OnPhoneOrder();
            discount = phoneorder.get_PaymentSection_DiscountAmount_OnPhoneOrderPage();
            tax = phoneorder.get_TaxAmount_PaymentSection();
            ...
            delayWithGivenTime(2000);
            String grandtotal = phoneorder.getGrandTotalAmount();
            ...
            delayWithGivenTime(2000);
            softassert.assertEquals(dashboardorder.getRecipientName_OnDeliveryPopup(), recifname1 + " " + recilname2, "Test Step 12 - Recipient name is not displayed on delivery popup");
            softassert.assertEquals(dashboardorder.getRecipientPhoneNum_OnDeliveryPopup(), "956-609-0756", "Test Step 12 - Recipient phone number is not displayed on delivery popup");
            delayWithGivenTime(2000);
            softassert.assertEquals(dashboardorder.getRecipientAddress_OnDeliveryPopup(), "1160 W 5th St Washington MO 63090 US", "Test Step 12 - Recipient address is not displayed on delivery popup");
            softassert.assertEquals(dashboardorder.getCustAndcompyNameOnDeliveryPopup(), "Hana_Sisterchicks | Abish David", "Test Step - 12 - customer and company name on delivery popup is not matched");
            delayWithGivenTime(2000);
            softassert.assertEquals(dashboardorder.getCustAddressOnDeliveryPopup(), "3402 Park Blvd", "Test Step - 12 -customer address on delivery popup is not matched");
            softassert.assertEquals(dashboardorder.getCustPhoneNumOnDeliveryPopup(), "956-655-0756", "Test Step - 12 - customer phone number on delivery popup is not matched");
            softassert.assertEquals(dashboardorder.getCustEmailOnDeliveryPopup(), "hanaposqateam@gmail.com", "Test Step - 12 - customer phone number on delivery popup is not matched");
            delayWithGivenTime(2000);
            softassert.assertEquals(dashboardorder.Verify_OrderType_Displayed_on_DeliveryPopup(), "Delivery Phone Order", "Test Step - 12 - Proper Order type is not displayed on delivery popup");
            softassert.assertEquals(dashboardorder.Verify_Top_Of_Invoice_OrderStatus(), "New", "Test Step - 12 - Order Status displayed on invoice popup in All order page");
            ...
            softassert.assertEquals(dashboardorder.get_Grand_Total(), "$" + grandtotal, "Test Step - 13 - Grand total is not displayed on invoice");
            softassert.assertEquals(dashboardorder.get_Paid_Amount(), "$0.00", "Test Step - 13 - Paid amount is not displayed on invoice");
            softassert.assertEquals(dashboardorder.get_Payment_Type(), "Invoice/House Account", "Test Step - 13 - Payment type is not displayed on invoice");
           ...
           /* delayWithGivenTime(2000);
            softassert.assertTrue(dashboardorder.verify_Delivery_Status_Table_Grid_IsDisplayed("New"), "Test Step - 11 - Delivery status table grid is not displayed on all orders page");
*/
        } catch (Exception e) {
            logger_Util = new LoggerUtil();
