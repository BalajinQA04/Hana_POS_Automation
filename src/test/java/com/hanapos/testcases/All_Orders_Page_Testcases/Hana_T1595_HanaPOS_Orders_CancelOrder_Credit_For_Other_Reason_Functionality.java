...
public class Hana_T1595_HanaPOS_Orders_CancelOrder_Credit_For_Other_Reason_Functionality extends TestBaseClass {
    ...
    public void Validate_Hana_T1595_HanaPOS_Orders_CancelOrder_Credit_For_Other_Reason_Functionality(
...
//========================= Above are pre - requisite steps for this test case =============================================
            ...
            delayWithGivenTime(1000);
            dashboardorder.Select_CancelOrder_Reason("Credit: Credit for other reason");
            delayWithGivenTime(2000);
            softassert.assertEquals(dashboardorder.get_cancelOrder_Reason(), "Credit: Credit for other reason", "Test Step - 5 - Cancel order reason is not displayed on cancel order popup");

            // Test Step - 16
            dashboardorder.Click_NoButton_CancelOrder_Popup();
            delayWithGivenTime(1000);
            softassert.assertEquals(dashboardorder.validate_Status_On_AllOrdersPage(invoiceNumber), "New", "Test Step - 8 - Cancelled order status is not displayed on order page after clicking the yes button on cancel order popup");
            ...
            softassert.assertTrue(
                    actualRefundMessage.contains("Refund Processed\nRefund Amount: $-" + total_amount + "\nRefund Method: Cash\nReason: Credit for other reason"),
                    "Refund message content mismatch for the Reason: Credit for other reason"
            );
